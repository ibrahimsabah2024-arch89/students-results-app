...
def decode_pdf_lines(pdf_path: str, gid_to_unicode: dict, arabic_font_prefix: str,
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for pi in range(total):
            ...
            clusters = []
            RECENT_WINDOW = 4
            ROW_TOLERANCE = 3.5  # widened from 2.0: on some exports, a row's
            # Arabic-name text and its Latin-digit number/average sit on
            # slightly different baselines (~2.5pt apart) due to font-metric
            # differences between the two scripts, which caused them to be
            # split into two separate lines instead of merged into one row.
            # Verified safe: the smallest gap between two genuinely different
            # rows in real exports is well above this (~9-12pt).
            for c in chars:
                placed = False
                for cluster in clusters[-RECENT_WINDOW:]:
                    if abs(cluster["top"] - c["top"]) <= ROW_TOLERANCE:
