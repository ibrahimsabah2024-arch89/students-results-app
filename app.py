"""
Streamlit app: منصة ISKM لتحليل البيانات وحساب الأعداد
------------------------------------------------------
...
from parser_school_exam import parse_school_exam_rows, summarize_school_exam

st.set_page_config(page_title="منصة ISKM لتحليل البيانات وحساب الأعداد", layout="wide", page_icon="📊")
...
st.markdown(
    """<div class="app-hero">
<h1>📊 منصة ISKM لتحليل البيانات وحساب الأعداد</h1>
...
def _process_pdf(file_bytes: bytes, progress_cb=None):
    """Do the heavy PDF decode/extraction work. NOT wrapped in
    st.cache_data -- see the manual session_state-based caching at the
    call site below, which lets us safely pass a progress_cb that calls
    real Streamlit UI commands (st.progress) as pages are processed.
    """
    ...
    decoded_ok = bool(gid_map)

    # IMPORTANT: always go through decode_pdf_lines, even when no broken CID
    # font was found. decode_pdf_lines does the row-clustering AND the
    # Arabic bidi-reordering (see font_decoder._bidi_reorder); pdfplumber's
    # own page.extract_text() does NOT reorder RTL text and was found to
    # produce fully reversed/garbled Arabic on some real "normal-font" PDFs
    # (e.g. "جامعة بغداد" coming out as "ةعماج دادغب"), which made every
    # keyword-based parser (search, medicine-college matching, etc.)
    # silently return zero results on those files -- no error, just an
    # empty tab. Passing an empty gid_to_unicode map + empty prefix makes
    # _decode_char() fall through to returning each char's own text
    # unchanged, so this is a no-op for the CID-decoding step itself and
    # only adds the (correct, needed) clustering/bidi pass.
    prefix = font_name.lstrip("/").split("+")[0] if font_name else ""
    lines = decode_pdf_lines(tmp_path, gid_map or {}, prefix, progress_callback=progress_cb)
...
@st.cache_data(show_spinner=False)
def _cached_school_exam_rows(file_bytes: bytes, _lines):
    """Same idea again, for the school-exam-results parser."""
    return parse_school_exam_rows(_lines)
...
if st.session_state.get("_pdf_hash") != file_hash:
    # New file (or first upload this session) -- actually process it,
    # with a real live progress bar. Safe to call st.progress here
    # because _process_pdf is a plain function, not st.cache_data-wrapped.
    ...
    lines, decoded_ok, font_name = _process_pdf(file_bytes, progress_cb=_report_progress)
    progress_placeholder.empty()

    # Cache in session_state so re-running the script (e.g. moving a
    # slider in another tab) doesn't reprocess the same file again.
    st.session_state["_pdf_hash"] = file_hash
    st.session_state["_pdf_lines"] = lines
    st.session_state["_pdf_decoded_ok"] = decoded_ok
    st.session_state["_pdf_font_name"] = font_name
else:
    lines = st.session_state["_pdf_lines"]
    decoded_ok = st.session_state["_pdf_decoded_ok"]
    font_name = st.session_state["_pdf_font_name"]
...
tab_search, tab_stats, tab_medicine, tab_school = st.tabs(
    ["🔍 البحث عن طالب", "📈 إحصائيات المعدلات", "🩺 كليات الطب", "🏫 نتائج امتحان مدرسي"]
)
...
# ----------------------------------------------------------------------
# TAB 4: School exam results (نتائج امتحان مدرسي)
# ----------------------------------------------------------------------
with tab_school:
    st.subheader("نتائج امتحان مدرسي -- نسبة النجاح وتوزيع الفئات")
    st.caption(
        "مخصص لملفات نتائج فحص الدراسة (تسلسل / الرقم الامتحاني / اسم الطالب / "
        "درجات المواد / النتيجة / المجموع / المعدل). يحسب عدد المشاركين، "
        "الناجحين، المعيدين، نسبة النجاح، وتوزيع الناجحين حسب فئة المعدل."
    )

    school_rows = _cached_school_exam_rows(file_bytes, lines)
    st.write(f"عدد صفوف الطلبة التي تم التعرف عليها في الملف: **{len(school_rows)}**")

    if not school_rows:
        st.warning("لم يتم العثور على صفوف بهذا التنسيق في الملف المرفوع.")
    else:
        summary = summarize_school_exam(school_rows)

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("👥 المشاركون", f"{summary['participants']:,}")
        c2.metric("✅ الناجحون", f"{summary['najeh_count']:,}")
        c3.metric("🔁 المعيدون", f"{summary['raseb_count']:,}")
        c4.metric("📊 نسبة النجاح", f"{summary['success_rate']:.2f}%")

        st.markdown("#### توزيع الناجحين حسب فئة المعدل")
        band_df = pd.DataFrame(
            {
                "الفئة": list(summary["band_counts"].keys()),
                "عدد الطلبة": list(summary["band_counts"].values()),
            }
        )
        st.bar_chart(band_df.set_index("الفئة"))
        st.dataframe(band_df, use_container_width=True, hide_index=True)

        with st.expander("📋 عرض كل الصفوف المستخرجة"):
            full_df = pd.DataFrame(
                [
                    {
                        "تسلسل": r.seq,
                        "الرقم الامتحاني": r.exam_no,
                        "الاسم": r.name,
                        "النتيجة": r.result,
                        "المعدل": r.average,
                        "المجموع": r.total,
                    }
                    for r in sorted(school_rows, key=lambda r: r.seq)
                ]
            )
            st.dataframe(full_df, use_container_width=True, height=500)

            school_buf = io.BytesIO()
            with pd.ExcelWriter(school_buf, engine="openpyxl") as writer:
                full_df.to_excel(writer, sheet_name="نتائج الامتحان", index=False)
            st.download_button(
                "⬇️ تنزيل الجدول الكامل كملف Excel",
                data=school_buf.getvalue(),
                file_name="نتائج_امتحان_مدرسي.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
